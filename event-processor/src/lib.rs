...
use crate::bindings::exports::timeline::event_processor::api::{Event, EventPredicate, EventStateResult, EventValue, Guest, LatestEventToStateResult, TimePeriod, WorkerId};
...
fn with_tl_has_existed<T>(f: impl FnOnce(&mut TLHasExistedTracker) -> Result<T, String>) -> Result<T, String> {
    let result = TL_HAS_EXISTED.with_borrow_mut(|state| {
        f(state)
    });
...
impl Guest for Component {
    ...
    fn initialize_tl_has_existed(worker: WorkerId, event_predicate: EventPredicate) -> Result<String, String> {
        ...
        with_latest_event_to_state(|state| {
         ...
         with_tl_has_existed(|state| {
            if let Some(predicate) = state.predicate.as_ref() {
                ...
                if state.state_dynamic_timeline.is_empty() || state.state_dynamic_timeline.future_is(false) {
                    let predicate_result =
                        predicate.evaluate(&GolemEvent::from_wit(event.clone()));

                    if predicate_result {
                        dbg!("Setting timeline as true from time {} since the predicate is true!", event.time);
